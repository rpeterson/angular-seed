...
angular.module("services.notifications", []).factory("notifications", [
  "$rootScope", function($rootScope) {
    ...
    $rootScope.$on("$routeChangeSuccess", function() {
      ...
      return notifications.ROUTE_NEXT.length = 0;
    });
    ...
    notificationsService.remove = function(notification) {
      return angular.forEach(notifications, function(notificationsByType) {
        var idx;
        idx = notificationsByType.indexOf(notification);
        if (idx > -1) {
          return notificationsByType.splice(idx, 1);
        }
      });
    };
    notificationsService.removeAll = function() {
      return angular.forEach(notifications, function(notificationsByType) {
        return notificationsByType.length = 0;
      });
    };
